...
public:
	...
	Bed (string ward, int number, bool occupied): ward(ward), number(number), occupied(occupied){}
...
public:
	Doctor(){}
	Doctor(string name) :Employee(name){}
...
public:
	...
	Patient(string name, int id, Doctor *d, Bed *b): name(name), id(id), d(d), b(b){}
	void report() const 
	{
		if (deleted) 
		{
